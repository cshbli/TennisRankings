...
def test_single_player():
    """
    Test the API with a single player (Aryna Sabalenka)
    """
    test_player = "Aryna Sabalenka"
...
def get_utr_ids_for_wta_players():
    """
    Read WTA rankings and fetch UTR player IDs for all players
    """
    print("Loading WTA Top 200 Rankings...")
    
    # Read WTA rankings
    df = pd.read_csv('WTA_Top_200_Rankings.csv')
    
    print(f"Found {len(df)} WTA players")
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[{idx+1}/{len(df)}] {player_name} (WTA #{rank})...", end=' ')
    ...
    output_file = f'WTA_Players_UTR_IDs_{current_date}.csv'
...
if __name__ == "__main__":
    print("="*60)
    print("WTA Players - UTR ID Fetcher")
    print("="*60 + "\n")
    
    # Test with single player first
    print("Step 1: Testing API with Aryna Sabalenka...\n")
    ...
    print("\n" + "="*60)
    print("Step 2: Fetching all WTA Top 200 players...")
    print("="*60 + "\n")
    
    results = get_utr_ids_for_wta_players()
