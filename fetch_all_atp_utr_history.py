...
INPUT_CSV = "ATP_Players_UTR_IDs_2026-02-04.csv"
OUTPUT_CSV = "ATP_Top_200_UTR_History_2021_2026.csv"
...
def parse_rating_history(stats_data, player_id, player_name, atp_rank):
    """
...
def fetch_all_utr_histories():
    """
    Main function to fetch UTR history for all ATP Top 200 players
    """
    print("="*80)
    print("Fetching UTR History for ATP Top 200 Players")
    print("="*80 + "\n")
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            print(f"[{idx+1}/200] Rank {atp_rank}: {player_name} - Invalid UTR ID")
            fail_count += 1
            continue
        
        print(f"[{idx+1}/200] Rank {atp_rank}: {player_name} (UTR ID: {utr_id})")
        ...
        if stats:
            ...
            history_data = parse_rating_history(stats, utr_id, player_name, atp_rank)
            
            if history_data:
                all_histories.append(history_data)
                print(f"  ✓ Success - {len([k for k in history_data.keys() if k not in ['ATP_Rank', 'Player_Name', 'UTR_ID']])} rating records")
                success_count += 1
            else:
    ...
    if all_histories:
        ...
        # Sort columns: ATP_Rank, Player_Name, UTR_ID, then dates in chronological order
        fixed_cols = ['ATP_Rank', 'Player_Name', 'UTR_ID']
